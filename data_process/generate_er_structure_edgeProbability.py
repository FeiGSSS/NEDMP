# -*- encoding: utf-8 -*-
'''
@File    :   generate_er_structure_data.py
@Time    :   2021/07/25 17:00:07
@Author  :   Fei gao 
...
if __name__ == "__main__":
    ...
    data_path = ["../data/er_structure/p{}".format(b) for b in range(10)]
    ...
    candidatesP = np.linspace(0.2, 0.6, 11)
    ...
    for sp, p in zip(save_path, range(10)):
        dataset = {"config":args,
                ...
                "simu_marginal":[],
                "dmp_marginal":[]}
        p_min, p_max = candidatesP[p], candidatesP[p+1]
        print("pmin = {} pmax = {}".format(p_min, p_max))
        for s in range(args.num_samples):
            t0 = time.time()
            # Random ER graph    
            for i in range(20):   
                graph = nx.erdos_renyi_graph(n=20, p=np.random.uniform(low=p_min, high=p_max))
