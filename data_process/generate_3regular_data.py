...
if __name__ == "__main__":
    ...
    data_path = ["../data/3regular/beta{:d}".format(b) for b in range(1, 13)]
    ...
    for gp, sp, beta in zip(graph_path, save_path, range(1, 13)):
        print("BETA:", beta)
        dataset = {"config":args,
                "graph":[],
                "seeds": [],
        ...
        if args.diffusion == "IC":
            node_prob = np.ones(N) * 1
            weight = np.ones(E) * (beta/18)
        elif args.diffusion == "SIR":
            node_prob = np.ones(N) * 0.5
            weight = np.ones(E) * (beta/24)
