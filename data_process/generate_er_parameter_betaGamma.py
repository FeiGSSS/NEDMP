...
if __name__ == "__main__":
    ...
    data_path = ["../data/er_parameter/beta{}".format(b) for b in range(10)] + ["../data/er_parameter/gamma{}".format(b) for b in range(10)]
    ...
    candidates_beta = np.linspace(0.05, 0.35, 11)
    candidates_gamma = np.linspace(0.3, 0.6, 11)
    
    for sp, cnt in zip(save_path, range(20)):
        dataset = {"config":args,
                "graph":[],
                "seeds": [],
                "node2edge": [],
        ...
        for s in range(args.num_samples):
            ...
            if cnt <=9:
                # beta
                node_prob = 0.5 * np.ones(N) 
                beta_min, beta_max = candidates_beta[cnt], candidates_beta[cnt+1]
                weight = np.random.uniform(low=beta_min, high=beta_max, size=E)
            else:
                cnt -= 10
                # gamma
                weight = np.ones(E) * 0.2
                gamma_min, gamma_max = candidates_gamma[cnt], candidates_gamma[cnt+1]
                node_prob = np.random.uniform(low=gamma_min, high=gamma_max, size=N)
